...
def joint_distribution_of_word_counts(texts, word0, word1):
    ...
    max1 = 0
    for text in texts:
      num0 = text.count(word0)
      num1 = text.count(word1)
      if (num0, num1) in Map.keys():
        Map[(num0, num1)] += 1
      else: Map[(num0, num1)] = 1
      if num0 > max0 : max0 = num0
      if num1 > max1 : max1 = num1
    Pjoint = np.zeros((max0 + 1, max1 + 1))
    for i in range(max0 + 1):
      for j in range(max1 + 1):
        if (i, j) in Map.keys(): Pjoint[i][j] = Map[(i, j)] / len(texts)
        else : Pjoint[i][j] = 0
...
def marginal_distribution_of_word_counts(Pjoint, index):
    ...
    '''
    if(index == 0) : Pmarginal=np.sum(Pjoint,axis=1) 
    else :  Pmarginal=np.sum(Pjoint,axis=0) 
...
def conditional_distribution_of_word_counts(Pjoint, Pmarginal):
    ...
    Pcond = np.copy(Pjoint)
    for i in range(len(Pjoint)):
      for j in range(len(Pjoint[i])):
        if(Pmarginal[i] == 0) : Pcond[i][j] = np.NAN
        else : Pcond[i][j] = Pjoint[i][j] / Pmarginal[i]
...
def covariance_from_distribution(P):
    ...
    '''
    # raise RuntimeError('You need to write this part!')
    Pmarginal0=np.sum(P,axis=1) 
    Pmarginal1=np.sum(P,axis=0) 
    covar = 0
    mu0 = mean_from_distribution(Pmarginal0)
    mu1 = mean_from_distribution(Pmarginal1)
    for i in range(len(P)):
      for j in range(len(P[i])):
        covar += (i - mu0) * (j - mu1) * P[i][j]
